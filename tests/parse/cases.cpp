...
TEST_CASE("parser:compound-multiply-assignment", "[parser]") {
  char stream[] = "y *= s;";
  std::stringstream source{stream};
  lex::Lexer l{source};
  Parser p{l};
  ...
  auto assignment_statement = dynamic_cast<AssignmentStatement*>(statement);
  REQUIRE(assignment_statement->assign_.type == lex::TokenType::STAR_EQ);
  REQUIRE(typeid(*assignment_statement->value_) == typeid(VarAccessExpression));
  REQUIRE(typeid(*assignment_statement->target_) == typeid(VarAccessExpression));

  auto value = dynamic_cast<VarAccessExpression*>(assignment_statement->value_);
  REQUIRE(value->name_.type == lex::TokenType::IDENTIFIER);
  REQUIRE(std::get<std::string_view>(value->name_.sem_info) == "s");

  auto target = dynamic_cast<VarAccessExpression*>(assignment_statement->target_);
  REQUIRE(target->name_.type == lex::TokenType::IDENTIFIER);
  REQUIRE(std::get<std::string_view>(target->name_.sem_info) == "y");
}
